---
id: recover-wrong-payment-fraud-kenya
...
![A bank statement and calculator on a desk](https://images.pexels.com/photos/4386370/pexels-photo-4386370.jpeg?auto=compress&cs=tinysrgb&w=1200 "The first move on any wrong payment is to check your own record, capture the transaction details, and flag it immediately. Evidence and speed are the whole game. Photo: Pexels")
...
```mermaid
flowchart TD
  A["You realise the payment is wrong"] --> B["Capture everything now:<br/>amount, time, reference, recipient details, screenshots"]
  B --> C{"Which rail?"}
  C -->|"M-Pesa"| D["Start the reversal at once:<br/>forward SMS to 456 / *456# / app / call 100 or 234"]
  C -->|"Bank (PesaLink/EFT)"| E["Call your bank's fraud line;<br/>request hold and recall; get a reference"]
  C -->|"RTGS / TT"| F["Call your bank immediately;<br/>request recall, but expect it may be final"]
  D --> G{"Fraud, not just an error?"}
  E --> G
  F --> G
  G -->|"Yes"| H["Report to police for an OB number;<br/>report to bank/Safaricom fraud;<br/>if SIM-related, call your telco to block the line"]
  G -->|"No, honest slip"| I["Follow up in writing; keep every reference"]
```
