---
id: 13-week-cash-forecast-kenya-sme
title: "The 13-Week Cash Forecast Every Kenyan SME Should Run"
summary: "Profit is an opinion; cash is a fact, and the fact that closes most Kenyan SMEs is running out of it while still profitable. A rolling 13-week cash forecast is the one spreadsheet that shows the squeeze before it arrives. This guide builds it column by column in Excel, adds the three scenarios that turn it into a decision tool, and shows how the same sheet sizes an overdraft or invoice limit before you ever walk into the bank."
...
![An entrepreneur working through business and banking paperwork](https://images.pexels.com/photos/4475523/pexels-photo-4475523.jpeg?auto=compress&cs=tinysrgb&w=1200 "The forecast is an hour a week with the numbers you already have. The discipline is the product, not the spreadsheet. Photo: Pexels")
...
```mermaid
flowchart LR
  A["Run the 13-week forecast"] --> B["Find the dip below your minimum line"]
  B --> C{"Shape of the gap?"}
  C -->|"Timing of collections"| D["Invoice / receivables limit<br/>sized to the dip"]
  C -->|"Recurring monthly swing"| E["Overdraft<br/>sized to the deepest point"]
  C -->|"Structural, never recovers"| F["Fix the model first,<br/>facility will not solve it"]
  D --> G["Take the sheet to the RM:<br/>size = depth, repayment = recovery"]
  E --> G
```
