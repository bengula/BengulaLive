---
id: seasonal-agri-working-capital-kenya
title: "Seasonal Working Capital for Agribusiness: Input Credit, Harvest Bridge, and Inventory Finance"
summary: "Farming and agri-trading run on a calendar that no ordinary loan is shaped for: cash goes out at planting, comes in at harvest, and a five-year term loan mismatches both. This guide maps the domestic seasonal cash cycle and the finance that actually fits it: input credit and merchant credit going in, harvest-bridge facilities coming out, and warehouse-receipt and inventory finance for holding produce off a glutted market. It stays on cash timing, not agronomy."
...
```mermaid
flowchart LR
  A["Pre-season<br/>Land prep, plan inputs"] --> B["Planting<br/>Cash OUT: seed, fertiliser, labour"]
  B --> C["Growing season<br/>Cash OUT: weeding, spraying, irrigation"]
  C --> D["Harvest<br/>Cash OUT: labour, transport, handling"]
  D --> E{"Sell now or store?"}
  E -->|"Sell at harvest"| F["Cash IN at the season's<br/>lowest price"]
  E -->|"Store and wait"| G["Warehouse receipt / inventory finance<br/>Cash IN now, sell later off-peak"]
  F --> H["Repay input + bridge finance"]
  G --> I["Sell weeks later at a better price"]
  I --> H
```
...
![Stored produce and stock records in a store](https://images.pexels.com/photos/4483610/pexels-photo-4483610.jpeg?auto=compress&cs=tinysrgb&w=1200 "Stored produce is not idle capital if you can borrow against it. A warehouse receipt turns a full store into cash while you wait for the price to recover. Photo: Pexels")
